var KeyInterceptPlugin = (function (patcher, metro) {
    'use strict';
  ...
  addEventListener("message", (e) => {
    ...
    */
    ...
    async function createNewConfig(userID) {
        ...
        await getSupabase().from("Sub_Config_Access").insert({
            "sub_id": userID,
            "config_id": configData.data.id
        });
        await getSupabase().from("Drone_Config").insert({
            "config_id": configData.data.id,
    ...
    async function getRules() {
        const rulesData = await getSupabase().from("Rules").select().eq("config_id", config.id).order("id", {
            ascending: false
        });
...
})(window.vendetta.patcher, window.vendetta.metro);
KeyInterceptPlugin;
